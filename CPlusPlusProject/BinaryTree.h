...
class TreeNode : public DataNode<T> {
	TreeNode(Key* key, T* value);
	Key* key;
	//defaults to red.
	bool isBlack = false;
	TreeNode<Key, T>* parentBranch;
	TreeNode<Key, T>* leftBranch;
	TreeNode<Key, T>* rightBranch;
	//int childrenCount = 0;

	TreeNode<Key, T>* getUncle();


	bool operator==(const TreeNode<Key, T>& other) {
...
template<class Key, class T> class RedBlackTree {
private:
	TreeNode<Key,T>* root;

	//void removeNode(TreeNode* node);

	void leftRotate(TreeNode<Key,T>* node);
	void rightRotate(TreeNode<Key,T>* node);

	static TreeNode<Key,T>* selectBranch(TreeNode<Key,T>* currentNode, Key* key);
